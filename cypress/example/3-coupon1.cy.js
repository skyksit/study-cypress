...
describe('쿠폰 테스트', () => {
  before(() => {
    ...
    cy.visit('https://mw.nsmall.com/customer/login/regular-member').then(s => {
      cy.get('.input_group').get('input').should('be.visible').as('input').eq(0).type('아이디입력', { log: false }).log('아이디 입력')
      // 패스워드 입력
      cy.get('@input').should('be.visible').eq(1).type('비밀번호입력', { force: true, log: false }).log('비밀번호 입력')
      // 로그인 버튼 클릭
  ...
  it('어떤 할인도 적용하지 않았을 때', () => {
    ...
    cy.get(':nth-child(9) > div > .coral_border > span').should('be.visible').click()
  ...
  it('어떤 할인율의 쿠폰을 사용했는지 확인', () => {
    ...
    cy.get('label.select > select').eq(2).find('option').each(sel => {
      // 쿠폰 출력
    ...
    cy.get('.price_current').then(orig => {
      ...
      cy.get('.total > dd > strong').then(txt => {
        const percent = Math.floor(((parseInt(original) - parseInt(txt.text().replace(',', ''))) / original) * 100)
        if (percent == 0) {
